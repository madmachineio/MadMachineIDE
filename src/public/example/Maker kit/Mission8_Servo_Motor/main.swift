...
func main() {
	let a0 = AnalogIn(.A0)        // reads the value of the potentiometer
    ...
    // it represents 180 degrees. In between, it represents the value from 0–180.
  	let servo = PWMOut(.PWM4, period: 20000, pulse: 1000)
  	
  	while true {
    	let value = a0.readPercent()
      	let pulse = UInt(500 + 2000 * value)
      	servo.set(period: 20000, pulse: pulse)        // sets the servo position according to the scaled value
